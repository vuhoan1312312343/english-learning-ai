...
import { getGoogleAuthUrl, authAPI } from "~/utils/api";
...
export const LoginScreen = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (loginScreenState === 'SIGNUP' && !name) {
    ...
    >
      ...
      <div className="flex grow items-center justify-center">
        <div className="flex w-full flex-col gap-5 sm:w-96">
          ...
          {error && (
            <div className="rounded-2xl bg-red-100 border-2 border-red-300 px-4 py-3 text-center text-red-700">
              {error}
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-2 text-black">
            ...
            <button
              type="submit"
              disabled={loading}
              className="rounded-2xl border-b-4 border-blue-500 bg-blue-400 py-3 font-bold uppercase text-white transition hover:brightness-110 disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loading ? 'Please wait...' : (loginScreenState === "LOGIN" ? "Log in" : "Create account")}
            </button>
          </form>
          ...
          <p className="text-center text-xs leading-5 text-gray-400">
            By signing in to LingoUp, you agree to our{" "}
            <Link
              className="font-bold"
