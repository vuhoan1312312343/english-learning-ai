...
export const RightBar = ({ hideGuestAuthCard = false }: { hideGuestAuthCard?: boolean }) => {
  ...
  // Check both old loggedIn state and new isAuthenticated
  ...
  return (
    <>
      <aside className="sticky top-0 hidden w-96 flex-col gap-6 self-start sm:flex">
        ...
        {userLoggedIn && lessonsCompleted < 10 ? (
        ...
        <DailyQuestsSection />
        <XpProgressSection />
        {!userLoggedIn && !hideGuestAuthCard && (
...
const LeaderboardRankSection = () => {
  ...
  const leaderboardLeague = "Bảng xếp hạng người dùng";
  return (
    <article className="flex flex-col gap-5 rounded-2xl border-2 border-gray-200 p-6 text-gray-700">
      ...
      <div className="flex gap-6">
        ...
        <div className="flex flex-col gap-5">
          ...
          <p className="text-sm leading-6 text-gray-500">
            Bạn đã tích lũy {xpThisWeek} XP trong tuần này
          </p>
        </div>
      </div>
    </article>
...
const CreateAProfileSection = ({
  ...
  const { isAuthenticated, user } = useAuth();
  const { logout } = useBoundStore((x) => ({ logout: x.logOut }));
      ...
      <article className="flex flex-col gap-5 rounded-2xl border-2 border-gray-200 p-6">
        <div className="flex items-center gap-4">
          {user.avatar ? (
            ...
            <div className="flex h-16 w-16 items-center justify-center rounded-full bg-gray-300 text-2xl font-bold text-white">
          ...
          <div className="flex flex-col">
            ...
            <span
              className={[
                "mt-1 inline-flex w-fit rounded-full border px-2 py-0.5 text-xs font-bold",
                getLevelBadgeClassName(user.level),
              ].join(" ")}
            >
              {normalizeLevel(user.level)}
            </span>
          </div>
        </div>
        <Link
          href="/profile"
          className="rounded-2xl border-b-4 border-blue-500 bg-blue-400 py-3 text-center font-bold uppercase text-white transition hover:border-blue-400 hover:bg-blue-300"
        >
          View Profile
        </Link>
        <button
          ...
          className="rounded-2xl border-2 border-gray-300 py-3 font-bold uppercase text-gray-600 transition hover:bg-gray-100"
        >
          Logout
        </button>
      </article>
